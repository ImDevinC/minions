...
function MatrixIcon(props: SvgIconProps) {
  return (
    <SvgIcon {...props} viewBox="0 0 24 24">
...
export function PlatformIcon({ platform, fontSize = "small" }: PlatformIconProps) {
  switch (platform) {
    case "github":
      return <GitHubIcon fontSize={fontSize} />;
    case "discord":
      return <DiscordIcon fontSize={fontSize} />;
    case "matrix":
      return <MatrixIcon fontSize={fontSize} />;
    default:
      return null;
  }
}
...
export function PlatformBadge({ platform, size = "small" }: PlatformBadgeProps) {
  ...
  return (
    <span title={`Source: ${label}`} style={{ color, display: "inline-flex" }}>
      <PlatformIcon platform={platform} fontSize={size === "small" ? "small" : "medium"} />
    </span>
  );
}
